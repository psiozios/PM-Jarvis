...
| **1** | `references/protocols/skill-memory.md` | The journal, and the loop back into method | When a run yields a durable learning about a skill |
| **1** | `references/eval-archetypes.md` | The six archetypes, their emphases, worked examples | When creating an `evals.md` or picking an archetype |
| **1** | `references/protocols/skill-patterns.md` | Reusable skill archetypes | Before building a radar / review / grooming skill |
| **1** | `references/protocols/evidence-ledger.md` | Lookup log, ledger, verdicts | Before the first lookup of a run that will drop a candidate |
...
**The layer-1 split is historical.** Fourteen files sit under `references/protocols/` and eight directly under `references/`. The table above is the index. **Do not move these files to tidy the split** — they are referenced by path from routine prompts, from `CLAUDE.md`, and from every skill, and a move breaks all of them silently.
...
**Deriving the `SKILL.md` ceiling — from this repo, not by analogy.** The fourteen automation-layer skills are the newest and most operationally precise in the tree. **Line counts below are a snapshot as of 2026-07-26** and several have since grown toward the ceiling; they are the evidence the budget was derived from, not a current census: `weekly-readahead` 101, `sync-doc` 102, `proactive-gaps` 110, `iterate-document` 113, `refinement-prep` 114, `chat-ingest` 115, `meeting-prep` 119, `monthly-review-fill` 122, `quarterly-review-fill` 124, `backlog-groom` 125, `weekly-review-fill` 125, `routine-responder` 127, `action-sweep` 130, `loose-threads` 131. They span 101-131 with a mean of 118 and carry more operational precision than any legacy skill at five times the length. Target 120 sits at that mean; ceiling 150 leaves headroom above the observed maximum without licensing a return to 400.

**Which layer-1 files are standards.** The two rows above are not interchangeable, so the classification has to be written down rather than guessed per audit. **Standards** — they carry catalogs and lookup tables that must stay resident: `config/house-style.md`, `references/protocols/prompt-architecture.md`, `references/absolute-rules.md`, `references/eval-archetypes.md`, `references/skill-chains.md`, `references/protocols/tracker-writes.md`. Everything else in layer 1 is a **protocol** and holds the 80/130 row. A file that outgrows its row either splits by concern or extracts its examples — it does not get reclassified to fit.
...
Concepts that **keep their own heading** because they are real and skill-specific: mode definitions (`## Mode: --<name>`), framework sections a skill owns and teaches, and worked examples the skill is the home of.

**`## Overview` and `## Purpose` are retired as spellings of the purpose paragraph, not as permission to leave a content intro unnamed.** Six skills used `## Overview` mid-file for something else entirely — a framework intro, a tools-and-timing block — which reads as a violation to any audit and tells the reader nothing. Name such a section for what it holds (`## The Three Timeframes`, `## Tools and Timing`). A heading inside a fenced output template is scaffolding and exempt; two skills' `## Purpose` and `## Overview` hits are exactly that and stay. A **specialized absolute-rule block** keeps its heading where rule 6 protects it — but as a bold lead-in rather than a `##`, so it stops occupying an outline slot in eighty-seven files.
