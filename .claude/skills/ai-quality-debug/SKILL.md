...
- `/feature-metrics` -> when the AI feature has no defined quality metric yet
- `/experiment-metrics` -> when you need a trustworthy metric to call an eval or A/B result
- `/root-cause-analysis` -> when quality regressed and the cause is unknown
- `/experiment-decision` -> when the go/no-go needs a test-versus-ship call
- `/analytics-instrumentation` -> when the quality signals you need are not being captured
